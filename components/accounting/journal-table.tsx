  ...
  if (journals.length === 0) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h3 className="text-lg font-semibold">Manual Journals</h3>
          ...
          <tbody className="divide-y divide-slate-200">
            {journals.map((journal) => (
              <tr key={journal.id}>
                ...
                <td className="px-4 py-3 text-slate-700">{journal.reference_number ?? "-"}</td>
                <td className="px-4 py-3 text-slate-700">{journal.description ?? "-"}</td>
                <td className="px-4 py-3 text-slate-700">{journal.line_summary}</td>
                <td className="px-4 py-3 text-slate-700">{formatCurrency(journal.debit_total)}</td>
                <td className="px-4 py-3 text-slate-700">{formatCurrency(journal.credit_total)}</td>
