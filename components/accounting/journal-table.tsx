...
export function JournalTable({ journals }: JournalTableProps) {
  if (journals.length === 0) {
    return (
      <section className="rounded-2xl border border-white/[0.08] bg-[#12151f] p-4 shadow-sm">
        ...
        <p className="mt-3 rounded-xl bg-white/[0.04] p-4 text-sm text-slate-400">No manual journals yet.</p>
      </section>
    );
  }
  ...
  return (
    <section className="rounded-2xl border border-white/[0.08] bg-[#12151f] p-4 shadow-sm">
      <div className="mb-3">
        <h3 className="text-base font-semibold">Manual Journals</h3>
        <p className="mt-1 text-sm text-slate-400">Simple two-line journal validation. Not full general ledger.</p>
      </div>
        ...
        <table className="w-full min-w-[787px] text-left text-sm">
          ...
          <tbody className="divide-y divide-white/[0.06]">
            {journals.map((journal) => (
              <tr key={journal.id}>
                <td className="px-3 py-2 text-slate-200">{journal.journal_date}</td>
                <td className="px-3 py-2 text-slate-200">{journal.reference_number ?? "-"}</td>
                <td className="px-3 py-2 text-slate-200">{journal.description ?? "-"}</td>
                <td className="px-3 py-2 text-slate-200">{journal.line_summary}</td>
                <td className="px-3 py-2 text-slate-200">{formatCurrency(journal.debit_total)}</td>
                <td className="px-3 py-2 text-slate-200">{formatCurrency(journal.credit_total)}</td>
