...
const PAGE = "/sales-delivery";
const optionalText = z.preprocess((value) => (value === "" ? null : value), z.string().nullable().optional());
const moneyInput = z.preprocess(parseCurrencyInput, z.number().min(0, "Selling price cannot be negative"));

const salesDeliverySchema = z.object({
  sales_date: z.string().min(1, "Order date is required"),
...
function resolveStatuses(quantity: number, delivered: number) {
  ...
  if (delivered <= 0) return { sales_status: "open" as const, delivery_status: "pending" as const, pending };
  if (pending > 0) return { sales_status: "partially_delivered" as const, delivery_status: "partially_delivered" as const, pending };
  return { sales_status: "delivered" as const, delivery_status: "delivered" as const, pending };
}
...
export async function createSalesDelivery(formData: FormData) {
  try {
    ...
    if (payload.quantity_delivered > payload.quantity) throw new Error("Delivered quantity cannot exceed sales quantity.");
    ...
    if (existingSuratJalan) throw new Error("Surat jalan number already exists. Use a new SJ number.");
    ...
    if (payload.so_number) {
      ...
      if (existingSalesOrder) throw new Error("Sales order number already exists. Use a new SO number.");
    }
    ...
    redirectSuccess(PAGE, `Sales & delivery ${payload.surat_jalan_number} saved.`);
  } catch (error) {
