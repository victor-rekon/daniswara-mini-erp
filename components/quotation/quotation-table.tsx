...
const statusClassName: Record<QuotationView["status"], string> = {
  draft: "bg-white/[0.04] text-slate-200",
        ...
        <table className="w-full min-w-[885px] text-left text-sm">
          ...
          <tbody className="divide-y divide-white/[0.06]">
            {quotations.map((quotation) => (
              <tr key={quotation.id}>
                <td className="px-3 py-2 font-semibold text-slate-100">{quotation.quotation_number}</td>
                <td className="px-3 py-2 text-slate-200">{quotation.quotation_date}</td>
                <td className="px-3 py-2 text-slate-200">{quotation.customer_name}</td>
                <td className="px-3 py-2 text-slate-200">{quotation.branch_name}</td>
                <td className="px-3 py-2 text-slate-200">{quotation.product_summary}</td>
                <td className="px-3 py-2 text-slate-200">{quotation.item_count}</td>
                <td className="px-3 py-2 text-slate-200">{formatNumber(quotation.total_quantity)}</td>
                <td className="px-3 py-2 text-slate-200">{formatCurrency(quotation.quotation_total)}</td>
                <td className="px-3 py-2">
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusClassName[quotation.status]}`}>
                    {quotation.status}
                  </span>
                </td>
                <td className="px-3 py-2 text-slate-200">{quotation.notes ?? "-"}</td>
              </tr>
            ))}
          </tbody>
        </table>
