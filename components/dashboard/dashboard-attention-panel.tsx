...
export function DashboardAttentionPanel({ summary }: { summary: DashboardSummary }) {
  const items = [
    {
    ...
    <section className="rounded-2xl border border-white/[0.07] surface p-5 shadow-card md:p-6">
      ...
      <div className="grid grid-cols-1 gap-2 xl:grid-cols-3">
        {items.map((item) => (
          <div
            key={item.label}
            className={`relative min-w-0 overflow-hidden rounded-xl border p-3.5 transition-shadow duration-200 ${
              item.isAlert
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p
                  className={`mt-1 whitespace-nowrap text-lg font-bold tabular-nums leading-tight ${
